...
// mapRowToCaso ahora en src/utils/mappers.ts

/**
 * Obtiene el conteo de mensajes entrantes para un caso.
 */
async function fetchMensajesCount(casoId: string): Promise<number> {
  ...
  if (error) {
    console.warn("[REALTIME] Error al contar mensajes:", error.message);
    return 0;
  }
...
/**
 * Obtiene el conteo de adjuntos sin procesar por IA para un caso.
 */
async function fetchAdjuntosPendientesCount(casoId: string): Promise<number> {
  const { data, error } = await supabase
    .from("adjuntos")
    .select("id")
    .eq("caso_id", casoId)
  ...
  if (error) {
    console.warn("[REALTIME] Error al contar adjuntos pendientes:", error.message);
    return 0;
  }
  return data?.length ?? 0;
}
 ...
 * Además computa mensajes_count y adjuntos_pendientes.
 */
async function fetchCasoCompleto(casoId: string): Promise<Caso | null> {
  ...
  const caso = mapRowToCaso(data as Record<string, unknown>);

  // Fetch counts in parallel
  const [mensajesCount, adjuntosPendientes] = await Promise.all([
    fetchMensajesCount(caso.id),
    fetchAdjuntosPendientesCount(caso.id),
  ]);
